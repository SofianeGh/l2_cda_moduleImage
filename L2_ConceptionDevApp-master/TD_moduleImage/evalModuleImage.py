...
msg("CMake = " + CMAKEFILE)
if MAKEFILE != "???" and CMAKEFILE != "???":
    msg("Vous avez un makefile et un cmake à la racine => je prends le makefile")
    CMAKEFILE = "???"
msg("==> note = " + str(NOTE))
msg("===> verification Makefile-CMake ... done")

### GENERATION DU MAKEFILE AVEC CMAKE ###
if CMAKEFILE != "???" and MAKEFILE == "???":
    msg("Utilisation de CMAKE")    
    if not isdir("build"):
...
if not isfile("data/image1.ppm"):
    ...
    with open("data/image1.ppm", 'r', encoding="iso-8859-1") as im_file_etu:
        ppm_magic_number = im_file_etu.readline()
        if ppm_magic_number[:2] == 'P3':
            if MODE == "PROF":
                if isfile("../image1.ppm"):
                    im_file_etu2 = open("data/image1.ppm", 'r')
                    image_etu = im_file_etu2.read()
            ...
            msg("le fichier image1.ppm doit être enregistré au format PPM ASCII, i.e., ouvrable dans un éditeur de texte, pour cela la fonction sauver() doit enregistrer au format PPM ASCII (format PPM \"P3\"), comme c'est le cas dans le code qui a été fourni", 0.5)
            
if not isfile("data/image2.ppm"):
    msg("image2.ppm non generee (ou pas dans data)", 0.5)
else:
    with open("data/image2.ppm", 'r', encoding="iso-8859-1") as im_file_etu:
        ppm_magic_number = im_file_etu.readline()
        if ppm_magic_number[:2] == 'P3':
            if MODE == "PROF":
                if isfile("../image2.ppm"):
                    im_file_etu2 = open("data/image2.ppm", 'r')
                    image_etu = im_file_etu2.read()
            ...
            msg("le fichier image2.ppm doit être enregistré au format PPM ASCII, i.e., ouvrable dans un éditeur de texte, pour cela la fonction sauver() doit enregistrer au format PPM ASCII (format PPM \"P3\"), comme c'est le cas dans le code qui a été fourni", 0.5)
...
###  README  ###
msg("===> readme ... (voir les rubriques attendues dans le sujet)")
...
###  DOCUMENTATION  ###
msg("===> documentation ...")
doxys = ["doc/doxyfile"]  #["doc/image.doxy", "doc/doxyfile"]
doxy = filein(doxys)
if doxy != "":
    rmfiles("doc/html")
    rmfiles("doc/latex")
...
for f in files:
    if isfile(f):
        ...
        if istart == -1:
            istart = texte.find(":: dessinerRectangle")
        iend = texte[istart+1:].find("::") + istart
        if texte[istart:iend].find("setPix") == -1 and texte[istart:iend].find("getPix") == -1:
            msg("dessinerRectangle doit appeller setPix/getPix itérativement",0.25)
